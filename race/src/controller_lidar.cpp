...
#define K_DG 55  // Difference of Gradient 상수
#define K_V 0.5 // velocity 상수


#define DETECT_RANGE 0.55 

#define MAX_DISTANCE 1.5	// FOWARD DISTANCE의 최대 탐지 거리
...
bool isExist(const obstacle_detector::Obstacles &data, geometry_msgs::Point &p) {
	...
	for(int i = 0; i < data.circles.size(); ++i) {
		if((data.circles[i].center.x < MAX_DISTANCE) &&
...
void calculator(const obstacle_detector::Obstacles data) {
	...
	geometry_msgs::Point c, front, leftP, rightP;
	race::drive_values msg;
	
	c.x = 0, c.y = 0;

	if(data.circles.size() == 0 || data.circles.size() == 1) {
		msg.steering = 100;
		msg.throttle = 1;
		pub.publish(msg);
		return;
	...
	rightP = data.circles[0].center;	
	leftP = data.circles[data.circles.size()-1].center;
	
	double distanceR = distance_from_car(rightP, c);
	double distanceL = distance_from_car(leftP, c);	
	double distanceF = sqrt(pow(front.x - c.x, 2) + pow(front.y - c.y, 2));

	double steer_pid_value = K_DG * (distanceR - distanceL);
	double speed_pid_value;

	if(!isExist(data, front) || (distanceR == distanceL)) { //go straight
		steer_pid_value = 0;
		speed_pid_value = K_V * distanceF + 3;
	}

	else if((distanceR > distanceL) || (rightP.y < 0)) { //turn right
		if(steer_pid_value > 100.0) steer_pid_value = 100.0;
		speed_pid_value = K_V * distanceF + 1;
	}

	else if((distanceR < distanceL) || (leftP.y > 0)) { //turn left
		if(steer_pid_value < -100.0) steer_pid_value = -100.0;
		speed_pid_value = K_V * distanceF + 1;
	}
