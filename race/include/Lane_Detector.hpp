...
void Lane_Detector::hough_to_cluster()
{
	...
	HoughLines(canny_park, lines_out_park, 1, CV_PI / 180, h_threshold); // left, right������ min, max theta ������
	...
	if (lines_out_park.size() >= 1)
	{
		for (size_t i = 0; i < lines_out_park.size(); i++)
		{
			...
			else
			{
				...
				{
					if (abs(different_rho[k][0] - deg) <= 5 && (different_rho[k][1] - rho)<55 || (abs(different_rho[k][0] - deg)>5 && abs(different_rho[k][0] - deg) <= 10) && abs(different_rho[k][1] - rho)<50)// �ι�° ���� ���� 35
					{
						cluster[k].sx += x1_;
						cluster[k].ex += x2_;
						cluster[k].sy += y1_;
...
void Lane_Detector::get_crosspoint()
{
	...
	for (int p = 0; p < clusterCount; p++)
	{
		...
		line(park_img, Point(cluster[p].sx, cluster[p].sy), Point(cluster[p].ex, cluster[p].ey), colortab[p], 3, LINE_AA);
	}
